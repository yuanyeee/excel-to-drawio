...
| 機能 | 内容 |
|------|------|
| 画像 (pic) | base64 埋め込みで再現 (SVG 代替 / EMF・WMF フォールバック対応) |
| 走査範囲 | シートデータから自動検出 (500 行超も可) |
| グリッドサイズ | 実データに合わせ動的拡張 (>500 行/列 OK) |
| 列幅計算 | OOXML 公式に準拠 (`defaultColWidth` 尊重) |
| 罫線スタイル | dashed/dotted/double/hair も対応 |
| 同色塗り内の内側罫線 | 同色隣接セル間の擬似罫線を抑制 |
| テキスト表示切れ | フォント自動縮小 + 空セルへの横延伸 |
| ハイパーリンク | セルリンクを保持 |
| 非表示行/列 | スキップオプション |
| 設定管理 | `ConvertConfig` dataclass |
| テキスト属性 | 回転・下線・取消線も対応 |
| 用途 | 汎用 Excel シート変換 |
