"""
Convert Excel XLSM sheet drawings + cell data to DrawIO XML.
Target sheet: ＮＥＴフロー(夜)_20250508

【修正履歴】
 v10 (2026-03): 【ラベル延伸幅の改善】
   - add_cell_labels: 塗りなしセルのテキストボックス横幅を制限
       * 従来: 塗りなしセルは値または塗りのあるセルが出現するまで右延伸
         → 枠外のラベルや白枠ラベルが横に極端に長くなる問題
       * 修正: テキスト量と文字サイズから必要幅を推定し、
         その幅に達したら延伸を停止する（塗りありセルは従来通り）

 v9 (2026-03): 【結合セル塗りつぶし修正】
   - add_cell_fills_merged: 結合セルの色を子セル全体に伝播
       * Excelでは結合セルの塗りは左上セル(topLeft)にのみ定義され、
         子セルはXMLに出現しない仕様のため、色グリッド構築後に
         build_merged_cell_mapsを使って全子セルへ色を伝播
       * これにより結合セル全体が正しい矩形として描画される

 v8 (2026-03): 【余計な枠・線の大幅削減】
   - add_cell_borders: 値なし+塗りなしセルの罫線描画をスキップ
       * 図外の空白領域に大量の細い矩形が出る問題を解消
   - add_cell_labels: offpage_markerロジックを削除
       * is_offpage_marker_label([A-Z]{1,2}\\d?) がノード識別子セルに
         誤ヒットし、五角形SVGを大量生成していた問題を解消
   - add_drawing_shapes: SVGイメージ方式廃止→DrawIO標準シェイプに変更
       * 先導/後続(homePlate/pentagon) → shape=offPageConnector
       * 複雑なパイプライン(collect_drawing_items等)を削除し直接emit_sp/walk_groupを使用

 v7 (2026-03): 【codex版統合・base64修正】
   - codex版の全改良をmain版へ反映
   - offpage_svg_data_uri: quote(URL形式) → base64エンコードに修正
       * DrawIOのスタイルパーサーは ';' で分割するためURL形式は危険
       * base64形式は DrawIO で確実に動作する

 v6 (2026-03): 【改良統合版】
   - codex版をベースに採用（mxGeometry の width/height バグ修正を含む）
   - main版の問題点を全修正:
       * mxGeometry: w/h → width/height（DrawIOで正しく描画されない問題を解決）
       * chars_px: +5の過剰パディング → 正確な計算に修正
   - 追加改善:
       * theme カラーの tint/shade 補正（近似計算）
       * 描画図形テキストの fontColor / fontStyle 抽出
       * grpSp 内 pic（画像）要素のスキップ処理
       * GEOM_STYLES に追加形状対応
       * セル塗りつぶしの重複描画防止（fill_grid と同期）
       * ページサイズ計算を全要素の最大座標で決定
       * 定数を設定ブロックにまとめて調整しやすく

 v5 (2026-03):
   - 隣接する同色セルを結合して大きな矩形として描画
   - theme/indexed カラーも処理
   - SCALE=1 に変更
   - ボーダー追加
 v4 (2025-06): セル背景色描画追加
 v3: grpSp座標系バグ修正 + schemeClr 対応
 v2: アンカー座標を使用
 v1: 初版
"""
...
class DrawioBuilder:
    ...
    def xml(self):
        return (
            ...
            + self.diagram_xml(diagram_id='d1')
            + '</mxfile>\n'
        )
...
def add_cell_fills_merged(sh_root, col_x, row_y, col_w_dict, row_h_dict, xf_fills, bld, log=None):
    if log is None:
        log = _log
    """
    ...
    log(f"  Color grid cells (after merge propagation): {len(color_grid)}")
    if not color_grid:
        return 0
    ...
    for row_el in sh_root.findall('.//x:row', ns):
        ...
        for cell in row_el.findall('x:c', ns):
            ...
            try:
                ...
                continue
            value_map[(r, c)] = read_cell_raw_text(cell, shared_strings)
    return value_map


def read_cell_raw_text(cell, shared_strings):
    ns = {'x': SS}
...
def add_cell_labels(sh_root, col_x, row_y, col_w_dict, row_h_dict, shared_strings,
    ...
    for row_el in sh_root.findall('.//x:row', ns):
        ...
        for cell in row_el.findall('x:c', ns):
            ...
            t = cell.attrib.get('t', '')
            raw_value = read_cell_raw_text(cell, shared_strings)
            if raw_value == '':
                continue
...
def suggest_output_path(input_path, sheet_name):
    ...
    safe_sheet = re.sub(r'[\\/:*?"<>|]+', '_', str(sheet_name)).strip() or 'output'
    return str(workbook_path.with_name(f'{safe_sheet}.drawio'))


def suggest_multi_output_path(input_path):
    _validate_workbook_suffix(input_path)
    workbook_path = Path(input_path)
    return str(workbook_path.with_suffix('.drawio'))


def convert_file(input_path, sheet_name, output_path, log_func=None):
    convert_sheets_to_file(input_path, [sheet_name], output_path, log_func=log_func)


def _prepare_workbook_resources(z):
    return {
...
def _build_sheet_diagram_xml(z, sheet_name, diagram_id, resources, log):
    sf, drw_path = find_paths(z, sheet_name)
    log(f"Sheet XML: {sf}")
    ...
    xf_fills = resources['xf_fills']
    xf_borders = resources['xf_borders']
    xf_text_styles = resources['xf_text_styles']
    xf_numfmts = resources['xf_numfmts']
    log(f"Fill styles:   {len(xf_fills)} xf indices")
    log(f"Border styles: {len(xf_borders)} xf indices")
    log(f"Text styles:   {len(xf_text_styles)} xf indices")
    log(f"Num formats:   {len(xf_numfmts)} xf indices")

    bld = DrawioBuilder(diagram_name=sheet_name)

    log("Processing cell fills (merging adjacent cells)...")
    fc = add_cell_fills_merged(sh_root, col_x, row_y, col_w_dict, row_h_dict,
                               xf_fills, bld, log=log)
    log(f"  Merged fill rectangles: {fc}")

    log("Processing cell borders...")
...
def convert_sheets_to_file(input_path, sheet_names, output_path, log_func=None):
    _validate_workbook_suffix(input_path)
    ...
    with zipfile.ZipFile(input_path, 'r') as z:
        resources = _prepare_workbook_resources(z)
        diagrams = []
        for index, sheet_name in enumerate(names, start=1):
            log(f"Processing sheet '{sheet_name}' ...")
            diagrams.append(_build_sheet_diagram_xml(z, sheet_name, f'd{index}', resources, log))

    xml_out = _build_mxfile(diagrams)
