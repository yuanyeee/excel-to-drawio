...
Excel (.xlsx / .xlsm) を Draw.io (.drawio) 形式に変換する**汎用ツール**の
**実験/開発用ブランチ**です。新機能はまずここで実装・検証し、安定確認後に
隣接フォルダ `ExcelToDrawIO/` へ反映する運用としています。

> 通常利用は `ExcelToDrawIO/` を使用してください。本フォルダは新機能の
> プレビューおよび開発検証用です。
...
|------|---------------------|------------------------------|
| 画像 (pic) | スキップ | base64 埋め込みで再現 (SVG 代替 / EMF・WMF フォールバック対応) |
| 走査範囲 | 固定 (270行x230列) | シートデータから自動検出 (500 行超も可) |
| グリッドサイズ | 固定 500×500 | 実データに合わせ動的拡張 (>500 行/列 OK) |
| 列幅計算 | 簡易式 | OOXML 公式に準拠 (`defaultColWidth` 尊重) |
| 罫線スタイル | thin/medium/thick のみ | dashed/dotted/double/hair も対応 |
| 同色塗り内の内側罫線 | そのまま描画 | 同色隣接セル間の擬似罫線を抑制 |
| テキスト表示切れ | - | フォント自動縮小 + 空セルへの横延伸 |
