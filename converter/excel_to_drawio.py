...
This implementation is self-contained and based on direct OOXML parsing:
- reads workbook/sheet relationships from XLSX zip,
- parses DrawingML (xdr) per sheet,
- emits draw.io XML without ExcelReader/DrawioWriter dependencies.
"""
...
# Ignore very common background/noise colors.
SKIP_FILL_COLORS = {
...
def _parse_drawing_xml(content: bytes) -> Tuple[List[DrawioShape], List[DrawioConnector]]:
    ...
    for anchor in root:
        ...
        for child in anchor:
            ...
            # grpSp/pic are intentionally skipped (too noisy in many files).

    return shapes, connectors


def _resolve_xl_target(base_path: str, target: str) -> str:
    # base_path: e.g. xl/worksheets/sheet1.xml
    base_dir = Path(base_path).parent
    joined = (base_dir / target).as_posix()
    # normalize ../ segments safely for zip paths
    parts: List[str] = []
...
def convert_excel_to_drawio(
    ...
    include_cells: bool = True,  # API compatibility for existing CLI/GUI.
) -> ConversionResult:
